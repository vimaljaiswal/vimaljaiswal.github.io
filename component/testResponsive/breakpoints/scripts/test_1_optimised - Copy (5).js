...
var resizedTimer;
  ...
  clearTimeout(resizedTimer);
          ...
          //  window.removeEventListener("orientationchange", onWindowResized);
       ...
       var resizedTimer =     setTimeout(function () {
           removeInitails([parentDivStack, lightBoxDivStack]); 
            ...
            count = -1;
             //console.log(orientation+"   >>>>>resize called");
      //  }
        ...
        for (var k = 0; k < parentDivStack.length; k++) {
            for (var i = 0; i < parentDivStack[k].length; i++) {
                ...
                for (var j = 0; j < subDiv.length; j++) {
                    ...
                    //  if ((parentDivStack[k][i].getAttribute('class') != 'PamphletLightboxPart')) {
                    //   if (!subDiv[j].getAttribute('class').match(/[\w-]*PamphletLightboxPart[\w-]*/g)) {
    ...
    function assignInitails(parentDivStack) {
        for (var k = 0; k < parentDivStack.length; k++) {
            for (var i = 0; i < parentDivStack[k].length; i++) {
                var subDiv = parentDivStack[k][i].querySelectorAll('div');
                for (var j = 0; j < subDiv.length; j++) {
                    //console.log(parentDivStack[k][i].getAttribute('class') +"   ??????????????????");
                    // if ((parentDivStack[k][i].getAttribute('class') == 'PamphletWidget') || (parentDivStack[k][i].getAttribute('class') == 'LightboxContent')) {
