(function () {
    ...
    function onWindowLoad() {
        ...
        var intervalSet = setInterval(function () {
            if (window.Muse.assets.checked) {
              //  document.getElementById("muse_css_mq").addEventListener('DOMSubtreeModified', LBresized);
    ...
    function LBresized() {
        clearTimeout(resizedTimerLB);
       // removeInitailsLB([lightBoxDivStack]);
        var resizedTimerLB = setTimeout(function () {
           //assignInitails([lightBoxDivStack]);
          //  assignProp([lightBoxDivStack]);
            document.getElementById("muse_css_mq").removeEventListener('DOMSubtreeModified', LBresized);
             clearTimeout(resizedTimerLB);
        }, setTime);

    }

    function removeInitailsLB(parentDivStack) {
        for (var k = 0; k < parentDivStack.length; k++) {
            for (var i = 0; i < parentDivStack[k].length; i++) {
                ...
                for (var j = 0; j < subDiv.length; j++) {
                    ...
                    elem.style.width = "";
                    elem.style.left = "";
                   if (subDiv[j].getAttribute('class').match(/[\w-]*museBGSize[\w-]*/g) ) {
        ...
        for (var k = 0; k < parentDivStack.length; k++) {
            for (var i = 0; i < parentDivStack[k].length; i++) {
                ...
                for (var j = 0; j < subDiv.length; j++) {
                    var elem = subDiv[j];
                    elem.style.width = "";
                    // if (!childDivs[j].getAttribute('class').match(/[\w-]*PamphletLightboxPart[\w-]*/g)) {
                    elem.style.left = "";
                    //  }
                }
            }
                    ...
                    if (childDivs[j].getAttribute('class').match(/[\w-]*museBGSize[\w-]*/g) || childDivs[j].getAttribute('class').match(/[\w-]*PamphletNextButton[\w-]*/g)) {
                        ...
                        if (childInitialLeft != undefined) {
                            ...
                            if (childInitialLeft == 'auto') {
                                //  if (!childDivs[j].getAttribute('class').match(/[\w-]*PamphletCloseButton PamphletLightboxPart popup_element museBGSize[\w-]*/g)) {
