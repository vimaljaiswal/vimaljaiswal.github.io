var vdxResponsivePlaylistWidget = (function() {
    ...
    EU.EventManager.add('VideoPlayer_e_play', function(event) {
        ...
        if (root.SOVCallback) {
            root.videoObj.videoID = root.currentVideoID;
            root.SOVCallback(root.videoObj);
        }        
    });
    ...
    EU.EventManager.add('VideoPlayer_e_ended', function(event) {
        ...
        if (root.EOVCallback) {
            root.videoObj.videoID = root.currentVideoID;
            root.EOVCallback(root.videoObj);
        }
        ...
        if (cvnum % root.totalVideos > 0) {
            ...
            if (root.EOPCallback) {
                root.EOPCallback(root.videoObj);
            }
        }
...
var vdxScrollEventHandlerWidget = (function() {
    ...
    function onCardChange(obj) {

        var cardInView = {};

        var higherPercentageCard = new Array();
        var upperCardDuplicate;
        for (var i = 0; i < obj.length; i++) {
            higherPercentageCard.push(obj[i].inViewPercent);
        }

        for (var j = 0; j < obj.length; j++) {
            console.log(obj[j].inViewPercent + " card inView  " + obj[j].name);
            if (obj[j].inViewPercent == Math.max.apply(Math, higherPercentageCard)) {
                cardInView = {
                    selectedCard: obj[j].name,
                    viewPercent: obj[j].inViewPercent
                };

                if (higherPercentageCard[j - 1] == obj[j].inViewPercent && j > 0) {
                   // upperCardDuplicate = obj[j].inViewPercent;
                   cardInView = {};
                    cardInView = {
                        selectedCard: obj[j-1].name,
                        viewPercent: obj[j-1].inViewPercent
                    };
                  console.log(obj[j].inViewPercent + "  .....counts   " + obj[j - 1].inViewPercent);
                }
            }

            if (cardInView.selectedCard != root.initialCard) {
                //  console.log(obj[j].name + "  name of max value of card");
                //  console.log(obj[j].inViewPercent + " Percent of  max value of card");
                root.DAPI.onCardChange(cardInView);
                root.initialCard = cardInView.selectedCard;
            }
    ...
    vdxScrollEventHandlerWidget.prototype.render = function() {
        ...
        dObj.onCardChange = onCardChange;
