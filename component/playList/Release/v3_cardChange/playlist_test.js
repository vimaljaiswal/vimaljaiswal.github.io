...
var vdxScrollEventHandlerWidget = (function() {
    ...
    function onScrollEnd(obj) {
        if (root.DAPI.onScrollStart) { root.DAPI.onScrollEnd(obj) }
        showElements();
        onCardChange(obj);
    }
        ...
        for (var propertyName in obj) {
            if (obj[0].inViewPercent != 100) {
                if (obj[0].inViewPercent > obj[1].inViewPercent) {
                    cardInView = obj[0].name;
                } else {
                    cardInView = obj[1].name;
                }
            } else {
                cardInView = obj[0].name;
            }
            if (cardInView != root.initialCard) {
                root.DAPI.onCardChange(cardInView);
                root.initialCard = cardInView;
            }
        }
    }
