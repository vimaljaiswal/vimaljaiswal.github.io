...
var vdxScrollEventHandlerWidget = (function() {
    ...
    function onCardChange(obj) {
        ...
        indexOfHighervalue = higherPercentageCard.indexOf(Math.max.apply(Math, higherPercentageCard));

        for (var j = 0; j < higherPercentageCard.length; j++) {
            if (higherPercentageCard[indexOfHighervalue] == higherPercentageCard[j]) {
                indexOfHighervalue = j
            }
        }
        
        cardInView = {
            selectedCard: obj[indexOfHighervalue].name,
            viewPercent: obj[indexOfHighervalue].inViewPercent
        };
